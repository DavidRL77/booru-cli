...
impl Commands {
    ...
    {
        match self {
            Commands::Url {client_args}
            => Self::url(client_args).await,
            Commands::Download { client_args, destination, clear }
    ...
    async fn url(client_args: ClientArgs)
    -> CommandResult
    {
    ...
    async fn download(client_args: ClientArgs, destination: &PathBuf, clear: bool) 
    -> CommandResult
    {
        if clear {
            Self::clear_temp().await?;
        ...
        reqwest::ClientBuilder::new()
            ...
            .default_headers(
                referer_header(referer_url(client_args.client))
            )
            .build()
    ...
    async fn json(client_args: ClientArgs, pretty: bool)
    -> CommandResult
    {
        let posts = Self::get_posts(client_args).await?;
    ...
    async fn get_posts(client_args: ClientArgs) 
    -> anyhow::Result<Vec<Box<dyn Post>>>
    {
